...
export function TripCard({ trip, variant, onDelete, onShare }) {
  ...
  const imageUrl = trip.cover_photo_url || "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=600&q=80";
    ...
    >
      ...
      <div className="relative h-[240px] w-full overflow-hidden bg-bg">
        <img 
          src={imageUrl} 
          alt={trip.name}
          className={`w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 ${isPast ? 'saturate-50 contrast-125 opacity-90' : ''}`}
        />
        ...
        <div className="absolute top-[12px] right-[12px]">
          ...
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="w-[18px] h-[18px]"><circle cx="12" cy="12" r="1"/><circle cx="12" cy="5" r="1"/><circle cx="12" cy="19" r="1"/></svg>
          </button>
          
          {showMenu && (
            <div className="absolute right-0 mt-[8px] w-[140px] bg-surface border border-border rounded-[12px] shadow-xl py-[8px] z-20">
              <button onClick={(e) => handleAction(e, 'edit')} className="w-full text-left px-[16px] py-[8px] text-[13px] text-ink hover:bg-bg transition-colors">Edit Trip</button>
              <button onClick={(e) => handleAction(e, 'share')} className="w-full text-left px-[16px] py-[8px] text-[13px] text-ink hover:bg-bg transition-colors">Share</button>
              <div className="h-[1px] bg-border my-[4px]"></div>
              <button onClick={(e) => handleAction(e, 'delete')} className="w-full text-left px-[16px] py-[8px] text-[13px] text-danger hover:bg-danger/10 transition-colors">Delete</button>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div className="text-[13px] text-ink h-[20px] overflow-hidden">
            ...
            <div className="transform translate-y-full opacity-0 group-hover:-translate-y-full group-hover:opacity-100 transition-all duration-300 font-['IBM_Plex_Mono'] text-[11.5px] font-semibold text-route">
              {trip.stopsCount || 1} stops · ${trip.totalCost?.toFixed(0) || 0} budget
            </div>
          </div>
        </div>
            ...
            <div className="w-full">
              ...
              <div className="flex items-center gap-[8px]">
                <div className="flex-1 h-[4px] rounded-full bg-border overflow-hidden">
                  <div className="h-full bg-route rounded-full" style={{ width: `${trip.progressPercent || 5}%` }}></div>
                </div>
                <span className="font-['IBM_Plex_Mono'] text-[10px] text-muted font-semibold">{trip.progressPercent || 5}%</span>
              </div>
            </div>
          )}
        </div>
  ...
  return (
    ...
    >
      <div className="w-[48px] h-[48px] rounded-full bg-bg border border-border flex items-center justify-center mb-[16px] group-hover:scale-110 transition-transform">
        <svg viewBox="0 0 24 24" fill="none" strokeWidth="2" strokeLinecap="round" className="w-[20px] h-[20px] stroke-currentColor"><path d="M12 5v14M5 12h14"/></svg>
      </div>
      <span className="font-['Fraunces'] text-[18px] font-semibold">Plan a New Trip</span>
      <span className="text-[13px] mt-[4px] opacity-70">Start your next adventure</span>
    </div>
  );
}
