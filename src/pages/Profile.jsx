...
export default function Profile() {
  ...
  const { upcoming, past, totalCountries } = useMemo(() => {
    ...
    upcomingList.sort((a, b) => new Date(a.start_date || 9999999999999) - new Date(b.start_date || 9999999999999));
    
    // Sort past: most recent first
    pastList.sort((a, b) => new Date(b.end_date || 0) - new Date(a.end_date || 0));
  ...
  const handleShare = (trip) => {
    ...
    if (trip.is_public) {
      const url = `${window.location.origin}/trip/public/${trip.public_slug || trip.id}`;
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-bg relative">
      ...
      <aside className="hidden lg:flex flex-col w-[220px] fixed inset-y-0 left-0 border-r border-border bg-bg p-[24px] z-20">
        <div className="flex items-center gap-[10px] mb-[40px] cursor-pointer" onClick={() => navigate('/dashboard')}>
        ...
        <nav className="flex flex-col gap-[8px] mb-[40px]">
          ...
          <Link to="/profile" className="font-['Inter'] font-medium text-ink bg-surface border border-border rounded-[8px] px-[16px] py-[10px] shadow-sm">Profile</Link>
        </nav>
      </aside>
      ...
      <div className="lg:hidden p-[18px] border-b border-border bg-surface flex items-center justify-between z-20 sticky top-0">
        <div className="flex items-center gap-[10px]" onClick={() => navigate('/dashboard')}>
          <div className="w-[34px] h-[34px] rounded-full relative shrink-0 overflow-hidden bg-route/10 flex items-center justify-center">
            <span className="font-['Fraunces'] font-bold text-route text-[18px]">G</span>
          </div>
      ...
      <main className="flex-1 lg:ml-[220px] p-[24px] lg:p-[48px] max-w-[1400px]">
        {loading ? (
          <div className="flex justify-center items-center h-[300px] text-muted font-['IBM_Plex_Mono']">Loading your travels...</div>
        ) : error ? (
          <div className="p-4 bg-danger/10 text-danger border border-danger/20 rounded-[12px]">{error}</div>
        ) : (
          <div className="animate-in fade-in duration-500">
            ...
            {hasNoTrips && (
              <div className="mt-[60px] flex flex-col items-center justify-center text-center p-[40px] border-2 border-dashed border-border rounded-[24px] bg-surface/50 max-w-[600px] mx-auto">
                ...
                <h2 className="font-['Fraunces'] text-[28px] font-semibold text-ink mb-[12px]">Your travel journal is empty</h2>
                <p className="text-[15px] text-muted mb-[32px] max-w-[400px]">
                  It's time to stop dreaming and start planning. Create your first itinerary to begin tracking your adventures.
                </p>
                <Link to="/trips/new" className="px-[24px] py-[12px] rounded-[12px] bg-route text-white font-medium hover:opacity-90 shadow-sm transition-opacity text-[15px]">
      ...
      <div className="lg:hidden fixed bottom-[26px] right-[24px] z-30">
        ...
        >
          <svg viewBox="0 0 24 24" fill="none" strokeLinecap="round" className="w-[24px] h-[24px] stroke-white stroke-[2]"><path d="M12 5v14M5 12h14"/></svg>
        </Link>
      </div>
